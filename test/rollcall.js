...
harness.test('@grr/rollcall', t => {
  ...
  logger.success('testing numeric detail', 665);
  t.is(logged.shift(), 'site:forge [SUCCESS] testing numeric detail');
  ...
  t.is(logged.shift(), 'site:forge [NOTICE]  testing array element detail');
  t.is(logged.shift(), '    [ 665 ]');

  logger.notice();
  t.is(logged.shift(), 'site:forge [NOTICE]  ---');

  logger.notice(new ErrorMessage('boom'));
  t.is(logged.shift(), 'site:forge [NOTICE]  Error: boom');

  logger.error(new ErrorMessage('boom'));
  t.is(logged.shift(), 'site:forge [ERROR]   boom');

  logger.notice(665);
  t.is(logged.shift(), 'site:forge [NOTICE]  Logged data:');
  t.is(logged.shift(), '    665');
  ...
  t.is(
    logged.shift(),
    `site:forge [SUCCESS] ${didIt} with no errors and no warnings`
  );
  ...
  logger.done({ files: 42, duration: 665 });
  t.is(logged.shift(), `site:forge [WARNING] ${didIt} with 3 warnings`);

  logger.done({ files: 42, duration: 665 });
  t.is(logged.shift(), `site:forge [WARNING] ${didIt} with 4 warnings`);
  ...
  t.is(
    logged.shift(),
    `site:forge [SUCCESS] Done! All 42 tests passed in 665ms`
  );
  ...
  t.is(
    logged.shift(),
    `{"label":"site:forge","level":"success","message":"Done",` +
