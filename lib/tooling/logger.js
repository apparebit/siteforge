...
const formatAsLines = (primary, ...rest) => {
  ...
  if (primary instanceof Error) {
    [diagnostic, ...explanation] = primary.message.split(EOL);
    let offset = primary.message.length + 1;
    if (primary.stack.charCodeAt(offset) < 32) offset++;
    detail = primary.stack.slice(offset).split(EOL);

    if (has(primary, 'cause')) {
      let cause;
      if (primary.cause instanceof Error) {
        cause = primary.cause.message.split(EOL);
      } else {
        cause = String(primary.cause.message).split(EOL);
      }
