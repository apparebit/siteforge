/**
 * @module task/buildHtaccess
 * @copyright (C) 2019 Robert Grimm
 */

import { options } from '../config.js';
import { createHash } from 'crypto';
import { directory, readFile, writeFile } from '../tooling/fs.js';
...
export default async function buildHTAccess() {
  ...
  const frontpage = await readFile(
    join(options.contentDir, 'index.html'),
    'utf8'
  );
  const hashes = hashInlineScripts(frontpage);

  // Inject into .htaccess.
  const htaccess = join(options.contentDir, '.htaccess');
